...
static std::ostream& operator<<(std::ostream& os, const OBJECT_SPECIFY& mode)
{
	switch (mode)
	{
		...
		case OBJECT_SPECIFY::OBJECT_FOLIAGE:
			os << "OBJECT_FOLIAGE"; break;
	}
	return os;
}
static std::stringstream& operator>>(std::stringstream& is, OBJECT_SPECIFY& mode)
{
	...
	else if (pos_str == "OBJECT_FOLIAGE")
		mode = OBJECT_SPECIFY::OBJECT_FOLIAGE;
	return is;
}
...
static D3D11_INPUT_ELEMENT_DESC layoutPNCTInstance[] =
{
	//SEMANTIC NAME, SEMANTIC INDEX, FORMAT, INPUT SLOT, ALIGNED BYTE OFFSET, INPUT SLOT CLASS, INSTANCE DATA STEP RATE, 
	{"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0},		//POSITION0을 의미
	{"NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0},
...
struct Cinema
{
	...
	{
		os << "szCinemaName:" << _towm(cinema.szCinemaName) << std::endl;
	...
	friend std::istream& operator>>(std::istream& is, Cinema& cinema)
	{
		...
		while (std::getline(is, cinemaLine) && cinemaLine != "")
		{
			...
			if (fieldName == "szCinemaName")
			{
				...
				std::string name;
				cinemaIss >> name;
				cinema.szCinemaName = _tomw(name);
			}
